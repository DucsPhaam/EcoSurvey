          ...
          {data?.leaderboard?.length >= 3 && (
            <div className="flex items-end justify-center gap-4 mb-8">
              ...
              <div className="text-center flex-1">
                <div className="w-14 h-14 rounded-full bg-gradient-to-br from-gray-200 to-gray-400 flex items-center justify-center mx-auto mb-2 shadow-lg text-gray-700 font-bold text-xl overflow-hidden border-2 border-earth-ink/20">
                  {data.leaderboard[1]?.avatar_url ? (
                    <img src={data.leaderboard[1].avatar_url} alt={data.leaderboard[1].full_name} className="w-full h-full object-cover" />
                  ) : (
              ...
              <div className="text-center flex-1">
                <div className="relative">
                  ...
                  <div className="w-18 h-18 rounded-full bg-gradient-to-br from-yellow-300 to-amber-500 flex items-center justify-center mx-auto mb-2 shadow-glow-green text-2xl font-bold text-yellow-900 overflow-hidden border-2 border-amber-400" style={{ width: 72, height: 72 }}>
                    {data.leaderboard[0]?.avatar_url ? (
                      <img src={data.leaderboard[0].avatar_url} alt={data.leaderboard[0].full_name} className="w-full h-full object-cover" />
                    ) : (
              ...
              {/* 3rd */}
              <div className="text-center flex-1">
                <div className="w-14 h-14 rounded-full bg-gradient-to-br from-orange-300 to-amber-600 flex items-center justify-center mx-auto mb-2 shadow-lg text-orange-900 font-bold text-xl overflow-hidden border-2 border-earth-ink/20">
                  {data.leaderboard[2]?.avatar_url ? (
                    <img src={data.leaderboard[2].avatar_url} alt={data.leaderboard[2].full_name} className="w-full h-full object-cover" />
                  ) : (
            ...
            {(data?.leaderboard || []).map((entry, i) => {
              ...
              return (
                <div key={entry.user_id}
                  ...
                  <div className="w-9 h-9 rounded-full bg-gradient-to-br from-brand-400 to-accent-400 flex items-center justify-center text-white font-bold text-sm flex-shrink-0 overflow-hidden border border-earth-ink/10">
                    {entry.avatar_url ? (
            ...
            {data?.my_rank && data.my_rank > 10 && (
              <div className="px-5 py-4 border-t-2 border-dashed dark:border-gray-700 bg-brand-50 dark:bg-brand-900/20">
                <div className="flex items-center gap-4">
                  ...
                  <div className="w-9 h-9 rounded-full bg-gradient-to-br from-brand-400 to-accent-400 flex items-center justify-center text-white font-bold text-sm overflow-hidden border border-earth-ink/10">
                    {user?.avatar_url ? (
