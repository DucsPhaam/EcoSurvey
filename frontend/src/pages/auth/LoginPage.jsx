      ...
      <main className="flex items-center justify-center p-6 sm:p-12 bg-paper-warm">
        <div className="w-full max-w-md animate-fade-in">
          <Link to="/" className="inline-flex items-center gap-2 mb-8 lg:hidden">
            ...
            <div className="card p-8 transition-shadow duration-300 hover:shadow-soft">
            <div className="flex items-center gap-2 mb-2">
              <span className="bg-earth-forest text-earth-paper px-2 py-0.5 font-mono text-xs uppercase tracking-widest animate-gentle-pulse">{t('auth:login')}</span>
            </div>
            ...
            <form onSubmit={handleSubmit} className="mt-8 space-y-5">
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  <button type="button" onClick={() => setShowPass(!showPass)}
                    ...
                    className="absolute right-3 top-1/2 -translate-y-1/2 text-earth-ink/60 hover:text-earth-ink transition-colors duration-200">
              ...
              <button type="submit" disabled={loading || (!captchaToken && !!import.meta.env.VITE_TURNSTILE_SITE_KEY)} className="btn-primary w-full disabled:opacity-70 disabled:cursor-not-allowed transition-all duration-300">
                {loading ? (
                  <span className="w-5 h-5 border-[3px] border-earth-paper/30 border-t-earth-paper animate-spin" />
                ) : (
