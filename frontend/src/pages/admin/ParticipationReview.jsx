        ...
        {['Pending', 'Approved', 'Rejected', ''].map((s) => (
          <button key={s} onClick={() => { setFilter(s); setPage(1) }}
            className={`px-4 py-2 rounded-xl text-sm font-semibold transition-all duration-200 ${
              filter === s 
                ? 'bg-earth-forest text-earth-cream shadow-brutal-sm' 
                : 'bg-earth-cream dark:bg-earth-ink/30 border-2 border-earth-ink/30 text-earth-ink dark:text-earth-paper hover:border-earth-forest hover:text-earth-forest'}`}>
            {s || 'All'}
          </button>
        ))}
        <button
